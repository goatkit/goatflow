...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "GoatFlow Migration Tool - Import OTRS data\n\n")
		fmt.Fprintf(os.Stderr, "Usage: %s [options]\n\n", filepath.Base(os.Args[0]))
		fmt.Fprintf(os.Stderr, "Commands:\n")
		fmt.Fprintf(os.Stderr, "  analyze   Analyze OTRS SQL dump file\n")
		fmt.Fprintf(os.Stderr, "  import    Import OTRS data to GoatFlow database\n")
		...
		fmt.Fprintf(os.Stderr, "  # Import to GoatFlow (dry run)\n")
		fmt.Fprintf(os.Stderr, "  %s -cmd=import -sql=otrs_dump.sql -db=postgres://user:pass@localhost/goatflow -dry-run\n\n", filepath.Base(os.Args[0]))
		fmt.Fprintf(os.Stderr, "  # Import to GoatFlow\n")
		fmt.Fprintf(os.Stderr, "  %s -cmd=import -sql=otrs_dump.sql -db=postgres://user:pass@localhost/goatflow\n\n", filepath.Base(os.Args[0]))
	}
...
func convertMySQLToPostgreSQL(mysqlSQL string) (string, string, error) {
	...
	// Skip tables that don't exist in GoatFlow schema or cause conflicts
