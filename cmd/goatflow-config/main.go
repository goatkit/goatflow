...
func main() {
	...
	storageDir := os.Getenv("GOATFLOW_CONFIG_DIR")
	if storageDir == "" {
		storageDir = "./config"
	}
...
func printUsage() {
	fmt.Println("🔧 GoatFlow Unified Configuration Manager")
	fmt.Println("======================================")
	fmt.Println("")
	fmt.Println("Manage all YAML configurations with version control, validation, and hot reload")
	fmt.Println("")
	fmt.Println("Usage: goatflow-config <command> [options]")
	fmt.Println("")
	...
	fmt.Println("  goatflow-config list config")
	fmt.Println("  goatflow-config show route health-checks")
	fmt.Println("  goatflow-config validate ./config/new-settings.yaml")
	fmt.Println("  goatflow-config lint ./routes/")
	fmt.Println("  goatflow-config version list config SystemID")
	fmt.Println("  goatflow-config rollback config SystemID v2025.1.15-1430")
	fmt.Println("  goatflow-config apply ./config/production.yaml")
	fmt.Println("")
	fmt.Println("Environment Variables:")
	fmt.Println("  GOATFLOW_CONFIG_DIR    Base directory for configurations (default: ./config)")
}
...
func handleShow(versionMgr *yamlmgmt.VersionManager) {
	if len(os.Args) < 4 {
		...
		fmt.Println("Usage: goatflow-config show <kind> <name>")
		os.Exit(1)
	}
...
func handleValidate(schemaRegistry *yamlmgmt.SchemaRegistry, linter *yamlmgmt.UniversalLinter) {
	if len(os.Args) < 3 {
		...
		fmt.Println("Usage: goatflow-config validate <file>")
		os.Exit(1)
	}
...
func handleLint(linter *yamlmgmt.UniversalLinter) {
	if len(os.Args) < 3 {
		...
		fmt.Println("Usage: goatflow-config lint <file|dir>")
		os.Exit(1)
	}
...
func handleVersion(versionMgr *yamlmgmt.VersionManager) {
	if len(os.Args) < 3 {
		...
		fmt.Println("Usage: goatflow-config version <list|show|create> [options]")
		os.Exit(1)
	}
	...
	case "list", "ls":
		if len(os.Args) < 5 {
			fmt.Println("Error: kind and name required")
			fmt.Println("Usage: goatflow-config version list <kind> <name>")
			os.Exit(1)
		}
	...
	case "show":
		if len(os.Args) < 6 {
			...
			fmt.Println("Usage: goatflow-config version show <kind> <name> <version>")
			os.Exit(1)
		}
...
func handleRollback(versionMgr *yamlmgmt.VersionManager) {
	if len(os.Args) < 5 {
		fmt.Println("Error: kind, name, and version required")
		fmt.Println("Usage: goatflow-config rollback <kind> <name> <version>")
		os.Exit(1)
	}
...
func handleDiff(versionMgr *yamlmgmt.VersionManager) {
	if len(os.Args) < 4 {
		fmt.Println("Error: kind and name required")
		fmt.Println("Usage: goatflow-config diff <kind> <name> [from] [to]")
		os.Exit(1)
	}
...
func handleApply(versionMgr *yamlmgmt.VersionManager, schemaRegistry *yamlmgmt.SchemaRegistry, linter *yamlmgmt.UniversalLinter) {
	if len(os.Args) < 3 {
		fmt.Println("Error: file path required")
		fmt.Println("Usage: goatflow-config apply <file>")
		os.Exit(1)
	}
...
func handleExport(versionMgr *yamlmgmt.VersionManager) {
	if len(os.Args) < 3 {
		...
		fmt.Println("Usage: goatflow-config export <kind> [output-dir]")
		os.Exit(1)
	}
...
func handleImport(versionMgr *yamlmgmt.VersionManager) {
	if len(os.Args) < 3 {
		...
		fmt.Println("Usage: goatflow-config import <dir>")
		os.Exit(1)
	}
