...
func main() {
	...
	var (
		...
		fsPath = flag.String("fs-path", getEnv("ARTICLE_STORAGE_FS_PATH", "/opt/goatflow/var/article"), "Filesystem storage path")
	...
	// Validate command
	if *command == "" {
		fmt.Println("Usage: goatflow-storage -command <switch|status|verify> [options]")
		fmt.Println("\nCommands:")
