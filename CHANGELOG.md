...
### Added
- **External Identity Provider Integration (OIDC Client)** — GoatFlow can now authenticate users via external identity providers (Google, generic OIDC). Built on the existing `AuthProvider` registry pattern with a new `OIDCProvider` interface (type-asserted in handlers) for redirect-based auth flows. PKCE (S256) mandatory on all OIDC flows; state tokens stored with 5-minute TTL via `MemoryStateStore`. Providers: `internal/platform/auth/oidc_provider.go` (generic OIDC), `internal/platform/auth/google_provider.go` (Google-specific discovery URL). Dependencies: `golang.org/x/oauth2` + `github.com/coreos/go-oidc/v3/oidc`.
- **OIDC auth handlers** — `GET /auth/:provider` (redirect to IdP) and `GET /auth/:provider/callback` (exchange code, verify ID token via JWKS, resolve/provision user, apply TOTP if enabled). Handlers in `internal/api/auth_oidc_handlers.go`, routes in `routes/auth.yaml`.
- **Admin identity provider management** — full CRUD at `/admin/identity-providers`: list, create, edit, update, delete, toggle enable/disable. Per-org configuration with org mapping table. Handlers in `internal/api/admin_identity_providers_handlers.go`, routes in `routes/admin.yaml`, templates in `templates/pages/admin/identity_providers.pongo2` + `identity_provider_form.pongo2`.
- **Login page IdP buttons** — dynamically rendered based on enabled providers for the org. Google icon and generic OIDC icon. Falls back to plain login form when no IdPs configured. Template: `templates/pages/login.pongo2`, handler: `internal/api/auth_htmx_handlers.go` (`IdPButtons` context variable).
- **Identity provider database schema** — `gk_identity_provider` (18 fields: name, type, client_id, client_secret, discovery_url, scopes, claims mapping, auto_provision, enabled) and `gk_identity_provider_org` (provider-to-org mapping). Migration 000023 for MySQL and PostgreSQL.
- **OIDC integration tests** — 5 integration tests with Keycloak 26.x testcontainer: discovery, full flow (authorize URL + PKCE), invalid state rejection, missing email rejection, PKCE challenge verification. Run via `make test-oidc-integration` (requires Docker socket). Unit tests for PKCE generation, provider name/priority, state store (6 tests).
- **15-language i18n for identity providers** — all ~40 `identity_providers.*` keys translated in all 15 language files (de, en, es, fa, fr, he, ja, pl, pt, ru, tlh, uk, ur, zh).
- **`test-oidc-integration` Makefile target** — runs OIDC integration tests in the toolbox container with Docker socket mounted (`--group-add` for permissions) and `--network host` for testcontainers/Keycloak access.
- **`HostAPI.GenerateThumbnail`** — new platform interface method (`pkg/plugin/plugin.go`) lets plugins request server-side thumbnail generation from image bytes. Signature: `GenerateThumbnail(ctx, data []byte, contentType string, maxWidth, maxHeight int) (thumbData []byte, thumbContentType string, err error)`. Wired across the full HostAPI stack: gRPC client (`pkg/plugin/grpcutil/hostapi.go`), gRPC server dispatch (`internal/platform/plugin/grpc/host_api.go`), `ProdHostAPI` (`filestorage.go`) with a `ThumbnailGenerator` adapter + `NewThumbnailGenerator(fn)` constructor (`hostapi_prod.go`), and `SandboxedHostAPI` delegation (`sandbox.go`). All 14 HostAPI implementations across 11 files updated (10 test mocks + 3 production + 1 adapter). Enabled in `cmd/goats/main.go` via `WithThumbnailService`, backed by `internal/service/thumbnail_service.go` (libvips via govips v2.16.0 — `NewImageFromBuffer` + `KernelLanczos3` resize + JPEG/PNG export). `vips.Startup` already runs at boot. An earlier TODO claiming these govips APIs were removed was incorrect — verified against the v2.16.0 source and the build passes.
- **TipTap image paste/drop extension** — the shared `static/js/tiptap-editor.js` (used by tickets, KB articles, and all rich-text surfaces) now handles clipboard paste and drag-drop of image files via a new `createImagePasteExtension(uploadUrl)` factory. When `imageUploadUrl` is configured, pasted/dropped images upload to that endpoint and insert as `<img src>` references; when unset, they fall back to inline base64 data URLs (bluemonday permits `data:` URLs). Rebuilt `static/js/tiptap-bundle.js`.
- **`imageUploadUrl` config option for `initTiptapEditor()`** — the opt-in endpoint that makes paste/drop upload rather than inline. Distinct from the extension itself: editors without an upload URL (e.g. ticket editors) keep the paste/drop handler but fall back to base64. The KB admin editor sets it to `/admin/kb/article/:id/attachments`.
- **Insert Image toolbar file-picker fallback** — the toolbar button now opens a native file picker when the URL prompt is cancelled, supporting both URL-based images and local file uploads.
- **Plugin uninstall endpoint** — `DELETE /api/v1/plugins/:name` (`HandlePluginUninstall`) shuts the plugin down, cleans up DB side-effects (custom fields, translations, UIs, cascade handlers), removes its directory from disk, and clears in-memory state. Registered under the admin plugin API group.
- **Admin dashboard search box** — a small search/filter input at the top-right of `/admin` that filters all admin module cards and sections in real-time via client-side JS (`adminFilter()`). Section headers also match; sections with no visible cards auto-hide. The placeholder and label reuse the existing `t("common.search")` i18n key so all 15 languages are covered without new translations.
- **Admin marketplace UI** — a full browse/install/update UI at `/admin/marketplace` with stats cards (Available, Installed, Updates), category/sort filters, and a searchable grid of community plugins. Backed by `GET /api/v1/plugins/marketplace` (index), `GET /api/v1/plugins/marketplace/search` (search), and `POST /api/v1/plugins/marketplace/install` (install with version-compat, dependency resolution, and signature verification). The `internal/platform/marketplace/client.go` implements `FetchIndex`, `ListInstalled`, `CheckUpdates`, `Install`, `Search`, and `FindPlugin`; download-verify-extract pipeline respects `GOATFLOW_REQUIRE_SIGNATURES` and `GOATFLOW_TRUSTED_KEYS`. Dashboard card at `/admin` links to the page. (`cli.go` routes in `internal/api/plugin_handlers.go`)
...
- **Form-encoded plugin bodies now parse into top-level args.** `buildPluginArgs` now expands `application/x-www-form-urlencoded` request bodies into top-level args (so plugins receive `{"title": "Hello"}` not `{"_body": "title=Hello"}`), matching the existing JSON-body behaviour. Lets plugins accept plain HTML form posts without a bespoke parser.
- **Widget handlers now receive full request context.** `HandlePluginWidget` calls the widget function with `buildPluginArgs(c, pluginName)` instead of `{}`, so widgets get `_org_id`, `_user_*`, and RBAC flags (`is_queue_admin`, etc.) like page/API handlers do. `GetPluginWidgets` consolidates its RBAC-context construction through the same helper.
- **FontAwesome icon rendering fixed in navigation and dashboard.** `templates/layouts/base.pongo2` and `templates/pages/admin/dashboard.pongo2` rendered `class="fa-solid fa-{{ pmi.Icon }}"`, but icons are stored with the full class (e.g. `fa-book`), producing `fa-fa-book`. Changed to `class="fa-solid {{ pmi.Icon }}"`. The admin dashboard also swapped emoji-style `<span>{{ item.Icon }}</span>` plugin icons for FontAwesome `<i>` for platform-wide consistency.
- **Dead host KB templates and routes removed.** Deleted `templates/pages/customer/{kb_article,kb_search,knowledge_base}.pongo2` and the `/kb`, `/kb/article/:id`, `/kb/search` entries in `routes/customer.yaml`. The KB plugin owns all customer KB routes via its own embedded templates, so the host copies were inert duplicates that only confused the template-coverage tests. `internal/platform/template/template_coverage_test.go` updated to drop the stale references.
- **JS build validates paste-extension wiring.** `make js-build` now greps the rebuilt bundle for the `createImagePasteExtension` definition, its call site with `config.imageUploadUrl`, and the `imageUploadUrl` option default — failing the build if any is missing. Guards against the "defined but never wired" regression that originally left paste/drop inert.
