...
- **Project Go toolchain bumped to 1.25.** `go.mod` directive, `Dockerfile` / `Dockerfile.config-manager` / `Dockerfile.goatkit` / `Dockerfile.route-tools` / `Dockerfile.tests` / `Dockerfile.toolbox` base images, the manual fallback download URL inside `Dockerfile.toolbox`, the `Makefile` `GO_IMAGE` default, the `actions/setup-go` version in `.github/workflows/test.yml`, the README badge, the SDK `sdk/go/go.mod` toolchain directive, and the design-doc workflow example all moved from 1.24 to 1.25. With the floor raised, the toolbox `govulncheck` pin (added earlier in this release as a 1.24 workaround) is now reverted to `latest`.
