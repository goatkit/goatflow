...
func saveRegistry() map[string]ProviderFactory {
    ...
    for k,v := range providerRegistry { cp[k]=v }
    return cp
}
func restoreRegistry(m map[string]ProviderFactory) { for k := range providerRegistry { delete(providerRegistry,k) }; for k,v := range m { providerRegistry[k]=v } }

// fake provider to test ordering without DB
type fakeProvider struct { n string; pr int; authenticated bool }
func (f *fakeProvider) Authenticate(ctx context.Context, u, pw string) (*models.User, error) { if f.authenticated { return &models.User{Login:u, Email:u, Role:"Agent", ValidID:1}, nil }; return nil, ErrInvalidCredentials }
func (f *fakeProvider) GetUser(ctx context.Context, id string) (*models.User, error) { return &models.User{Login:id, Email:id, Role:"Agent", ValidID:1}, nil }
func (f *fakeProvider) ValidateToken(ctx context.Context, t string) (*models.User, error) { return nil, ErrAuthBackendFailed }
...
func TestProviderOrderFromConfig(t *testing.T) {
    ...
    if len(got) != 2 { t.Fatalf("expected 2 providers, got %d", len(got)) }
    if got[0][:4] != "beta" { t.Fatalf("expected beta first, got %v", got) }
}
