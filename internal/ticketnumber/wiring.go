...
func SetupFromConfig(configDir string) SetupResult {
    ...
    if v, err := adapter.GetConfigValue("SystemID"); err == nil {
        switch tv := v.(type) {
        case int: systemID = strconv.Itoa(tv)
        case int64: systemID = strconv.FormatInt(tv, 10)
        case float64: systemID = strconv.Itoa(int(tv))
        case string: if tv != "" { systemID = tv }
        }
    }
    if v, err := adapter.GetConfigValue("Ticket::NumberGenerator"); err == nil {
        if s, ok := v.(string); ok && s != "" { generatorName = s }
    }
