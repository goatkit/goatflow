...
func (s *DBStore) Add(ctx context.Context, dateScoped bool, offset int64) (int64, error) {
    ...
    if database.IsPostgreSQL() {
        ...
        q := `INSERT INTO ticket_number_counter (counter, counter_uid, create_time)
              ...
              RETURNING counter`
        var c int64
        if err := s.db.QueryRowContext(ctx, q, uid, offset).Scan(&c); err != nil { return 0, err }
        return c, nil
    }
    if database.IsMySQL() {
    ...
    case nil:
        ...
        if _, err = tx.ExecContext(ctx, database.ConvertPlaceholders(`UPDATE ticket_number_counter SET counter=$2 WHERE counter_uid=$1`), uid, newVal); err != nil { return 0, err }
        if err = tx.Commit(); err != nil { return 0, err }
        return newVal, nil
    case sql.ErrNoRows:
        if _, err = tx.ExecContext(ctx, database.ConvertPlaceholders(`INSERT INTO ticket_number_counter (counter, counter_uid, create_time) VALUES ($2, $1, NOW())`), uid, offset); err != nil { return 0, err }
        if err = tx.Commit(); err != nil { return 0, err }
        return offset, nil
    default:
        return 0, scanErr
