...
func TestAllPagesSSR(t *testing.T) {
    ...
    if st, err := os.Stat(templatesDir); err == nil && st.IsDir() {
        _ = os.Setenv("TEMPLATES_DIR", templatesDir)
    }
    ...
    if len(tplFailures) > 0 {
        if strict {
            for _, f := range tplFailures { t.Errorf("template invalid/missing: %s", f) }
            t.Fatalf("%d template(s) invalid or missing (from YAML)", len(tplFailures))
        } else {
            for _, f := range tplFailures { t.Logf("template invalid/missing (non-strict): %s", f) }
        }
    }
    ...
    for _, doc := range docs {
        ...
        for _, rt := range doc.Spec.Routes {
            if strings.ToUpper(rt.Method) != "GET" { continue }
            if rt.Template == "" { continue }
            if rt.RedirectTo != "" || rt.Websocket { continue }
    ...
    for _, pg := range pages {
        ...
        if err != nil { t.Fatalf("build request %s: %v", pg.path, err) }
